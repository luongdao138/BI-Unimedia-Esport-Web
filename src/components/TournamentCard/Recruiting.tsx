...
const TournamentCardRecruiting: React.FC<Props> = ({ tournament }) => {
  ...
  return (
    <ESCard>
      <ESCardMedia cornerIcon={<Icon className="fas fa-trophy" fontSize="small" />} image={attr.avatar}></ESCardMedia>
      <ESCardContent>
        ...
        <Box display="flex" flexDirection="row" mt={1} alignItems="center">
          ...
          <Box ml={1} color={Colors.white}>
            <Typography variant="caption">{attr.start_date}</Typography>
          </Box>
        </Box>
        <Box display="flex" flexDirection="row" mt={1} alignItems="center">
          <ESChip
...
export default TournamentCardRecruiting
