...
const WinnerSelector: React.FC<WinnerSelectorProps> = ({ src, ratio, is_required, cancel, onUpdate, onRemove }) => {
  ...
  const [imageDimesion, setImageDimesion] = useState<{ width: number; height: number }>(null)
  ...
  useEffect(() => {
    if (zoom < 1 && imageDimesion !== null) {
      const { width, height } = imageDimesion
    ...
    if (f) {
      ...
      reader.onload = (e) => {
        ...
        img.onload = function () {
          ...
          setImgLoad(true)
          if (zoom < 1) {
            setImageDimesion({ width: w, height: h })
            setCroppedAreaPixels({ width: w, height: h, x: 0, y: 0 })
          }
        }
  ...
  const renderPreviewImage = () => {
    if (zoom < 1) {
      return (
        <AspectRatio ratio="16/9" className={classes.imageWrapper}>
          <img src={file} className={classes.imageContainer} />
        </AspectRatio>
      )
    }
    return (
          ...
          {file && imgLoad ? (
            ...
            <div className={classes.imageContainer}>
              {imgSrc ? (
                ...
                <Image height="148" width="116" src="/images/big_logo.png" />
              )}
