...
const webSocketMiddle = () => {
  ...
  return (store: StoreType) => (next: AppDispatch) => <A extends Action>(action: A): A => {
    const connect = () => {
      const currentUser = store.getState().auth.user
      closeExisting()
      const uri = `${process.env.CHAT_END_POINT}/?accessToken=${currentUser?.accessToken}&deviceId=${DEVICE_ID}&appVersion=web_v2`
