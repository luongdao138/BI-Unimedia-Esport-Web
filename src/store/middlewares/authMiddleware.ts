...
import { getArenaWinners, getTournamentParticipants, getTournamentMatches } from '@store/arena/actions/index'
...
export const authMiddleware: Middleware = (store: StoreType) => (next: AppDispatch) => <A extends Action>(action: A): A => {
  ...
  const arenaTypes = [getArenaWinners.rejected.type, getTournamentParticipants.rejected.type, getTournamentMatches.rejected.type]
