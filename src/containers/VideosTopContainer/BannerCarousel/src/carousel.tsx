...
import { props, state, slideInfoMap, staticSlideInfo, renderedSlide } from './interfaces'

export default class StackedCarousel extends React.PureComponent<props, state> {
  static defaultScaleFactor = 0.85
  static defaultTransitionTime = 450
  ...
  private validateProps = () => {
    const { currentVisibleSlide, maxVisibleSlide, fadeDistance, customScales, data } = this.props
  ...
  private initializeProperties = (constructor = false) => {
    this.validateProps()
    const { carouselWidth, slideWidth, currentVisibleSlide: currentVisibleDisplaySlide, maxVisibleSlide } = this.props
    ...
    const { offsets, scaledOffsets, scales } = this.calculateScaleAndOffsets(slidePerSide)
  ...
  private calculateScaleAndOffsets = (slidePerSide: number) => {
    const { carouselWidth, slideWidth, customScales, fadeDistance } = this.props
    ...
    for (let slide = 1; slide <= slidePerSide; slide++) {
      const scale = customScales ? customScales[slide] : Math.pow(StackedCarousel.defaultScaleFactor, slide)
      scales.push(scale)
    ...
    for (let slide = 1; slide <= slidePerSide; slide++) {
      ...
      scaledOffsets[slide] = isCustomFade ? (fadeDistance as number) * availableSpace : slideWidth * scale * offSetPercentage
  ...
  constructor(props: props) {
    super(props)
    const { renderedSlides, slideInfoMap, slidePerSide, sortedSlideInfo, centerPosition, renderedSlidePerSide } = this.initializeProperties(
      true
    )
  ...
  componentDidUpdate(prevProps: props) {
    ...
    const { slideInfoMap, slidePerSide, newRenderedSlides, sortedSlideInfo, centerPosition } = this.initializeProperties()
  ...
  private updateHeight = () => {
    ...
    const parsedHeight = parseInt(window.getComputedStyle(slideDOM as Element).height)
  ...
  private debouncedClearInvisibleSlide = () => {
    ...
    this.clearSlideTimeout = setTimeout(
      ...
      transitionTime !== undefined ? transitionTime : StackedCarousel.defaultTransitionTime
    )
  }
  ...
  private clearUselessSlide = () => {
    ...
    const newRenderedSlides = renderedSlides.filter(({ slideIndex, dataIndex }) => {
  ...
  private moveCarousel = (steps: number, disableSwipeRightState = false) => {
    const { renderedSlides } = this.state
    const { onActiveSlideChange } = this.props
    ...
    if (steps !== 0) {
      ...
      for (
        let newSlideIndex = steps > 0 ? this.slidePerSide - steps + 1 : -this.slidePerSide - steps - 1;
        ...
        if (!newSlides.find(({ slideIndex }) => slideIndex === newSlideIndex)) {
          ...
          const insertDataIndex = this.modDataRange(newCenterDataIndex + newSlideIndex)
          this.keyCount += 1
  ...
  private getSlideDOMInfo = (slideIndex: number) => {
    ...
    const slideDOM = listRef.getElementsByClassName(`react-stacked-center-carousel-slide-${slideIndex}`)[0]
  ...
  private getSlideScaleAndOpacity = (centerOffset: number) => {
    ...
    const { position, slideIndex, scale, maxTransformDistance, maxTransformScale, maxTransformOpacity, opacity } = targetSlide
    const offset = Math.ceil(position) - centerOffset
    const offsetPercentage = maxTransformDistance.left === 0 ? 0 : (offset / maxTransformDistance.left) * (slideIndex <= 0 ? -1 : 1)
  ...
  private onSwipeStart = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    for (const slide of renderedSlides) {
      ...
      const { maxTransformDistance, position } = this.safeGetSlideInfo(newSlideIndex)
  ...
  private onSwipe = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const newRenderedSlides = prevRenderedSlides.map((slide, index) => {
      ...
      const maxDeltaX = swipePositionInfo[index][swipeLeft ? 'maxLeft' : 'maxRight']

      const targetPosition = position + (swipeLeft ? -1 : 1) * maxDeltaX * transformPercentage
  ...
  private resetSlides = () => {
    ...
    const newDefault = renderedSlides.map((slide) => {
      const { opacity, scale, position } = this.safeGetSlideInfo(slide.slideIndex)
      return {
        ...slide,
  ...
  render() {
    ...
    const cursor = (useGrabCursor && (swipeStarted ? 'grabbing' : 'grab')) || 'default'
    return (
      <div
      ...
      >
        {renderedSlides.map(({ opacity, slideIndex, dataIndex, position, scale, key, zIndex }) => {
          ...
          const transition = swipeStarted ? 'none' : customTransition || `all ${transitionTime}ms ease, z-index ${zDuration}ms ease`
          const isCenterSlide = tempShift ? zIndex === this.maxZIndex : slideIndex === 0
          return (
            <div
              key={key}
            ...
            >
              {dataIndex !== -1 && (
                <Component dataIndex={dataIndex} data={data} slideIndex={slideIndex} isCenterSlide={isCenterSlide} swipeTo={this.swipeTo} />
              )}
            </div>
          )
        })}
