...
export const getInitialLiveSettingValues = (data?: LiveStreamSetting): FormLiveType => ({
  ...
  stepSettingTwo: {
    ...
    re_category: data ? data.category : -1,
    date_time_notification_delivery: data ? data.stream_notify_time : null,
    date_time_schedule_delivery_start: data ? data.stream_schedule_start_time : null,
    date_time_schedule_end: data ? data.stream_schedule_end_time : null,
    re_use_ticket: data ? (data.use_ticket === 1 ? true : false) : false,
    re_ticket_price: data ? data.ticket_price : 0,
    date_time_ticket_sale_start: data ? data.sell_ticket_start_time : null,
